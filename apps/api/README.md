...
`ALLOWED_SIGN_IN`. Google, Microsoft and Zoho OAuth pairs are optional connection
capabilities, never sign-in methods. Users authenticate with local email/password or
an IdP configured in **Settings → SSO**, then link any number of provider accounts in
