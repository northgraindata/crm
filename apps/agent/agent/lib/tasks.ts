...
export async function claimDue(
	...
	const claimed = await db.$queryRaw<LeasedTask[]>`
		...
		WHERE t.id = due.id
			RETURNING t.id, t."contactId", t."companyId", t."surveyResponseId", t.kind, t.reason,
...
export async function retireExhausted(): Promise<TaskSubject[]> {
	...
	return db.$queryRaw<TaskSubject[]>`
		...
		RETURNING t.id, t."contactId", t."companyId", t."surveyResponseId", t.kind;
	`;
}
...
export async function scheduleTask(input: {
	...
	const existing = await db.agentTask.findFirst({
		where: {
			...
			surveyResponseId: input.surveyResponseId ?? undefined,
		},
		select: { id: true },
	});
