...
async function load() {
  ...
  byId("api-url").value = saved.apiUrl ?? "http://localhost:3001";
  byId("token").value = saved.token ?? "";
  const tab = await getActiveTab();
  if (tab?.url?.includes("linkedin.com/in/")) {
    byId("status").textContent = "LinkedIn profile URL detected. Confirm the fields manually.";
  } else {
    byId("status").textContent = "Open a LinkedIn profile, then use the extension button.";
  }
}
...
byId("capture-form").addEventListener("submit", async (event) => {
  ...
  try {
    const tab = await getActiveTab();
    if (!tab?.url?.includes("linkedin.com/in/")) throw new Error("The active tab is not a LinkedIn profile.");
    ...
    status.textContent = result.taskId ? "Saved and follow-up created." : "Saved to Northgrain.";
  } catch (error) {
    status.textContent = error instanceof Error ? error.message : "Could not save this contact.";
  }
});
